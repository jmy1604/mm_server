...
func (this *TcpConn) SendData(type_id uint16, data []byte) {
	defer func() {
		if err := recover(); err != nil {
...
func (this *TcpConn) event_recv(data []byte) {
	...
	if this.closed || this.closing {
		log.Error("event_recv return %v %v", this.closed, this.closing)
		return
	}
...
func (this *TcpConn) recv_loop() {
	...
	for {
		...
		if io.EOF == err {
			log.Trace("[%v][玩家%v]另一端关闭了连接", this.addr, this.T)
		...
		for mb.Len() >= 3 {
			...
			if l > 0 && mb.Len() >= int(l)+extra_len {
				...
				if this.closing || this.closed {
					log.Info("recv loop quit when closing")
					return
				}

				//log.Info("发送给处理函数 %v", d)
				this.event_recv(d)
