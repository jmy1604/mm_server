...
func (this *TcpConn) Send(msg proto.Message, immediate bool) {
	if msg == nil {
...
func (this *TcpConn) on_recv(d []byte) {
	...
	for {
		if len(data) < 4 {
			log.Error("[ip:%v] 消息长度小于7", this.addr)
			this.event_disc(E_DISCONNECT_REASON_PACKET_MALFORMED)
			return
		}
		...
		if len(data) < 4+int(ml) {
			log.Error("[ip:%v] 消息数据长度不够", this.addr)
			this.event_disc(E_DISCONNECT_REASON_PACKET_MALFORMED)
			return
		}
...
func (this *Node) SetHandler(type_id uint16, typ reflect.Type, h Handler) {
	...
	if has {
		log.Warn("[%v]消息处理函数已设置,将被替换 %v %v", this.addr, type_id, typ)
	}
