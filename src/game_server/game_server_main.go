...
func main() {
	...
	if !server_config.ServerConfigLoad("game_server.json", &config) {
	...
	// 初始化大厅
	if !game_server.Init() {
		log.Error("game server init failed !")
		return
	} else {
		log.Info("game server init succeed !")
	}
	...
	game_server.Start(config.UseHttps)
}
