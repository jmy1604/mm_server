...
const (
	UID_TOKEN_KEY = "mm:game_server:uid_token_key"
)
...
func (this *LoginTokenMgr) LoadRedisData() int32 {
	string_map, err := redis.StringMap(game_server.redis_conn.Do("HGETALL", UID_TOKEN_KEY))
	if err != nil {
...
func _save_redis_login_token(uid, token string, now_time time.Time, player_id int32) {
	...
	err = game_server.redis_conn.Post("HSET", UID_TOKEN_KEY, uid, string(bytes))
	if err != nil {
