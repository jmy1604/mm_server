...
func google_pay_save(order_id, bundle_id, account string, player *Player) {
	...
	err = game_server.redis_conn.Post("HSET", GOOGLE_PAY_REDIS_KEY, order_id, string(bytes))
	if err != nil {
...
func check_google_order_exist(order_id string) bool {
	exist, err := redis.Int(game_server.redis_conn.Do("HEXISTS", GOOGLE_PAY_REDIS_KEY, order_id))
	if err != nil {
...
func apple_pay_save(order_id, bundle_id, account string, player *Player) {
	...
	err = game_server.redis_conn.Post("HSET", APPLE_PAY_REDIS_KEY, order_id, string(bytes))
	if err != nil {
...
func check_apple_order_exist(order_id string) bool {
	exist, err := redis.Int(game_server.redis_conn.Do("HEXISTS", APPLE_PAY_REDIS_KEY, order_id))
	if err != nil {
		log.Error("redis do err %v", err.Error())
		return false
	}
...
func verify_google_purchase_data(player *Player, bundle_id string, purchase_data, signature_data []byte) int32 {
	...
	log.Trace("Player %v purchase_data %v unmarshaled: %v", player.Id, purchase_data, *data)
