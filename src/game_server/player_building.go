...
func (this *Player) ChkMapBlock() (count int32) {
	...
	log.Debug("玩家[%d]自动刷新障碍检查 %d %d", this.Id, time.Unix(int64(last_up_unix), 0).Format("2006-01-02 15:04:05.999999999"), time.Unix(int64(cur_unix), 0).Format("2006-01-02 15:04:05.999999999"))
	for tmp_unix := last_up_unix; tmp_unix+global_config.MapBlockRefleshSec < cur_unix; tmp_unix += global_config.MapBlockRefleshSec {
		...
		if nil != new_building {
			log.Debug("玩家[%d]自动刷新障碍检查尝试增加障碍 %d  %d", this.Id, tmp_block.Id, new_building.Id)
