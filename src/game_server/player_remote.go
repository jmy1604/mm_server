...
type rpc_mfunc func(int32, []int32, []byte) ([]byte, int32)
...
func RemoteGetUsePB(from_player_id, object_type, object_id int32, req_msg_id int32, req_msg proto.Message, resp_msg proto.Message) (err_code int32) {
	...
	result_data, err_code = rpc_proto.RpcCommonGet(game_server.rpc_client, "G2G_CommonProc.Get", from_player_id, object_type, object_id, req_msg_id, req_data)
	if err_code < 0 {
		return
	}
...
func remote_get_multi_player_info(from_player_id int32, to_player_ids []int32) (resp *msg_rpc_message.G2GPlayerMultiInfoResponse, err_code int32) {
	...
	datas := rpc_proto.RpcCommonMultiGet(game_server.rpc_client, "G2G_CommonProc.MultiGet", from_player_id, rpc_proto.OBJECT_TYPE_PLAYER, to_player_ids, int32(msg_rpc_message.MSGID_G2G_PLAYER_MULTI_INFO_REQUEST), req_data)
...
// 获取多个玩家信息返回
func remote_get_multi_player_info_response(from_player_id int32, to_player_ids []int32, req_data []byte) (resp_data []byte, err_code int32) {
	var req msg_rpc_message.G2GPlayerMultiInfoRequest
	err := _unmarshal_msg(req_data, &req)
	if err != nil {
		err_code = -1
