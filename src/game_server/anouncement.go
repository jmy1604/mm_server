...
func (this *AnouncementMgr) PushNew(msg_type int32, is_broadcast bool, player_id int32, player_name string, player_level int32, param1, param2, param3 int32, text string) bool {
	...
	if is_broadcast {
		...
		if player != nil {
			//player.rpc_call_anouncement(msg_type, param1, text)
		}
	}

	log.Debug("Pushed Anouncement all_anounce[%v] msg_type[%v] player_id[%v] player_name[%v] player_level[%v] param1[%v] param2[%v]", this.msg_num, msg_type, player_id, player_name, player_level, param1, param2)
...
func (this *Player) CheckAndAnouncement() int32 {
	now_time := int32(time.Now().Unix())
	if now_time-this.db.Anouncement.GetLastSendTime() < global_config.AnouncementSendCooldown {
		return -1
	}
	items := anouncement_mgr.GetSome(this, global_config.AnouncementSendMaxNum)
	if items != nil && len(items) > 0 {
		this.db.Anouncement.SetLastSendTime(now_time)
		notify := &msg_client_message.S2CAnouncementNotify{
			Items: items,
		}
		this.Send(uint16(msg_client_message.S2CAnouncementNotify_ProtoID), notify)
	}
	return 1
}
