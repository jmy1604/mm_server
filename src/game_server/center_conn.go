...
func (this *CenterConnection) OnConnect(c *server_conn.ServerConn) {
	...
	notify := &msg_server_message.G2CGameServerRegister{}
	...
	c.Send(uint16(msg_server_message.MSGID_G2C_GAME_SERVER_REGISTER), notify, true)
}
...
func (this *CenterConnection) RegisterMsgHandler() {
	...
	this.SetMessageHandler(uint16(msg_server_message.MSGID_C2G_LOGIN_SERVER_LIST), C2GLoginServerListHandler)
	this.SetMessageHandler(uint16(msg_server_message.MSGID_C2G_NEW_LOGIN_SERVER_ADD), C2GNewLoginServerAddHandler)
	this.SetMessageHandler(uint16(msg_server_message.MSGID_C2G_LOGIN_SERVER_REMOVE), C2GLoginServerRemoveHandler)
}

func center_conn_msgid2msg(msg_id uint16) proto.Message {
	if msg_id == uint16(msg_server_message.MSGID_C2G_LOGIN_SERVER_LIST) {
		return &msg_server_message.C2GLoginServerList{}
	} else if msg_id == uint16(msg_server_message.MSGID_C2G_NEW_LOGIN_SERVER_ADD) {
		return &msg_server_message.C2GNewLoginServerAdd{}
	} else if msg_id == uint16(msg_server_message.MSGID_C2G_LOGIN_SERVER_REMOVE) {
		return &msg_server_message.C2GLoginServerRemove{}
	} else {
...
func C2GLoginServerListHandler(conn *server_conn.ServerConn, msg proto.Message) {
	req := msg.(*msg_server_message.C2GLoginServerList)
	if nil == conn || nil == req {
		log.Error("C2GLoginServerListHandler param error !")
		return
	}
...
func C2GNewLoginServerAddHandler(conn *server_conn.ServerConn, msg proto.Message) {
	req := msg.(*msg_server_message.C2GNewLoginServerAdd)
	if nil == conn || nil == req || nil == req.GetServer() {
		log.Error("C2GNewLoginServerAddHandler param error !")
		return
	}
...
func C2GLoginServerRemoveHandler(conn *server_conn.ServerConn, msg proto.Message) {
	req := msg.(*msg_server_message.C2GLoginServerRemove)
	if nil == conn || nil == req {
		log.Error("C2GLoginServerRemoveHandler param error !")
		return
	}
	...
	if nil != cur_login {
		log.Info("C2GLoginServerRemoveHandler 登录服务器[%d]连接还在，断开连接", serverid)
		cur_login.ForceClose(true)
