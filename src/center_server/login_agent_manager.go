...
func (this *LoginAgentManager) OnDisconnect(c *server_conn.ServerConn, reason server_conn.E_DISCONNECT_REASON) {
	...
	if c.T > 0 {
		...
		rm_notify := &msg_server_message.C2GLoginServerRemove{}
		rm_notify.ServerId = c.T
		game_agent_mgr.Broadcast(uint16(msg_server_message.MSGID_C2G_LOGIN_SERVER_REMOVE), rm_notify)
	}
...
func L2CLoginServerRegisterHandler(conn *server_conn.ServerConn, msg proto.Message) {
	...
	add_notify := &msg_server_message.C2GNewLoginServerAdd{}
	...
	game_agent_mgr.Broadcast(uint16(msg_server_message.MSGID_C2G_NEW_LOGIN_SERVER_ADD), add_notify)
