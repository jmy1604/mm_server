...
func (this *SignalMgr) DoAllCloseFunc() {
	...
	select {
	case sig, ok := <-this.signal_c:
		{
			this.b_closing = true

			log.Info("Recv close_sig !!", sig, ok)
			this.b_closing = true
			for mod_name, val := range this.close_map {
				log.Info("Close %s", mod_name)
				go val.close_func(val)
			}

			this.WaitAllCloseOver()
